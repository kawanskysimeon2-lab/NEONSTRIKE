...
async function registerUser(username, password) {
  ...
  if (db) {
    const { data: existing } = await db.from('players').select('username').eq('username', username).maybeSingle();
...
async function loginUser(username, password) {
  ...
  if (db) {
    const { data, error } = await db.from('players').select('*').eq('username', username).maybeSingle();
...
async function savePlayerData() {
  ...
  if (db) {
    const { error } = await db.from('players').update({ data: playerData }).eq('username', currentUser.username);
...
async function loadGlobalConfig() {
  if (db) {
    const { data } = await db.from('game_config').select('*').eq('id', 1).maybeSingle();
    if (data && data.config) {
      globalConfig = { ...defaultGlobalConfig(), ...data.config };
    } else {
      // cria linha inicial
      await db.from('game_config').insert({ id: 1, config: defaultGlobalConfig() });
...
async function saveGlobalConfig() {
  if (db) {
    const { error } = await db.from('game_config').update({ config: globalConfig }).eq('id', 1);
...
function subscribeRealtime() {
  ...
  realtimeChannel = db.channel('game_config_changes')
...
async function tryFullscreenLandscape() {
  ...
  } catch (e) { /* usuário pode ter bloqueado, segue sem tela cheia */ }
  try {
    if (screen.orientation && screen.orientation.lock) {
      await screen.orientation.lock('landscape');
    }
  } catch (e) { /* navegador pode não suportar fora de fullscreen */ }
}

// Sair da tela cheia / destravar orientação ao voltar pro menu
...
async function loadAdminPlayers() {
  if (db) {
    const { data, error } = await db.from('players').select('username, data');
...
async function adminSavePlayer(username, btnEl) {
  ...
  if (db) {
    const { error } = await db.from('players').update({ data: player.data }).eq('username', username);
...
async function refreshAdminStats() {
  ...
  if (db) {
    const { data } = await db.from('players').select('data');
